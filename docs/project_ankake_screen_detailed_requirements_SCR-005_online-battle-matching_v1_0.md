...
ユーザー名、合言葉、選択デッキ、マッチング情報および一時プレイヤーIDを永続保存しない。ユーザー名と合言葉は、画面内の遷移に必要な期間だけ保持する。
...
| AREA-004 | 使用デッキ領域 | 選択中のデッキ概要とデッキ選択操作を表示する |
...
| PNL-001 | 使用デッキパネル | パネル | 選択中デッキの名前、枚数、対戦可否およびプレビューを表示する |
| BTN-002 | デッキ選択ボタン | ボタン | OVL-010を表示する |
...
| OVL-010 | オンライン対戦用デッキ選択ダイアログ | ダイアログ | オンライン対戦で使用する保存済みデッキを選択する |
...
OVL-010では保存済みデッキを更新日時の降順で表示する。対戦可能なデッキのみを選択可能とし、対戦不可能なデッキは一覧に表示して理由を併記する。

デッキを選択する前にマッチングが開始されることを防ぐため、OVL-010の決定操作後に開始可否を再評価する。
...
|---|---|---|---|
| 通常表示 | 画面内で保持 | 保持しない | 保持しない |
| 待機キャンセル成功 | 画面内で保持 | 破棄する | 保持しない |
| マッチング成立 | SCR-004へ引き継ぐ | SCR-004へ一時的に引き継ぐ | SCR-004へ引き継ぐ |
| メニューへ戻る | 破棄する | 破棄する | 破棄する |
| 対戦結果から戻る | 画面内で保持 | 破棄する | 保持しない |

ブラウザの永続保存領域、URL、ログおよび画面キャプチャ用の補助テキストに、合言葉、一時プレイヤーIDまたは対戦IDを保存・表示してはならない。
