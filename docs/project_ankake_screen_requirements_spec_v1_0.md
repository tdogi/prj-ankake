...
リタイアした対戦では、リタイアしたプレイヤーを敗北、相手を勝利として対戦結果オーバーレイを表示する。
...
flowchart TD
    ...
    BATTLE -->|リタイアを選択| RESULT[OVL-006<br/>対戦結果]
    ...
    ONLINE_BATTLE -->|リタイアを選択| ONLINE_RESULT
...
| TRN-008 | 対戦画面        | リタイアを選択する                  | 対戦結果オーバーレイ |
...
| TRN-020 | 対戦画面（オンライン対戦） | リタイアを選択する | 対戦結果オーバーレイ |
...
対戦中にリタイアを選択したプレイヤーは敗北し、相手プレイヤーは勝利する。確認ダイアログは表示せず、通常の対戦結果オーバーレイを表示する。
...
リタイアを選択した場合、バックエンドへリタイア要求を送信する。リタイアしたプレイヤーを敗北、相手プレイヤーを勝利として確定・同期し、対戦結果オーバーレイを表示する。
